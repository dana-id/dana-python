...
class QueryMerchantInfoRequest(BaseModel, BaseSdkModel):
    """
    Business fields mapped to `request.body` in the JSON envelope (head/signature are handled by the client/runtime).
    """ # noqa: E501
    role_id: StrictStr = Field(description="Login identifier value (format depends on `loginType`, e.g. `{countryCode}-{mobileNo}` when `loginType` is `MOBILE_NO`)")
    login_type: StrictStr = Field(description="Login identifier type used with `roleId`")
    is_query_account: Optional[StrictBool] = Field(default=None, description="When true, include merchant account list in the response")
    __properties: ClassVar[List[str]] = ["roleId", "loginType", "isQueryAccount"]

    @field_validator('login_type')
    def login_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ROLE', 'MOBILE_NO']):
            raise ValueError("must be one of enum values ('ROLE', 'MOBILE_NO')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        if "QueryMerchantInfoRequest".endswith("Response") and "response" in obj and "body" in obj["response"]:
            # For OPEN_API nested format, pass the nested response structure directly
            _obj = cls.model_validate({
                "roleId": obj.get("roleId"),
                "loginType": obj.get("loginType"),
                "isQueryAccount": obj.get("isQueryAccount")
            })
        else:
