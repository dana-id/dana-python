...
class MerchantResourceInformation(BaseModel, BaseSdkModel):
    """
    MerchantResourceInformation
    """ # noqa: E501
    resource_type: StrictStr = Field(description="Type of merchant resource")
    value: StrictStr = Field(description="JSON string containing amount and currency information")
    __properties: ClassVar[List[str]] = ["resourceType", "value"]

    @field_validator('resource_type')
    def resource_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['MERCHANT_DEPOSIT_BALANCE', 'MERCHANT_AVAILABLE_BALANCE', 'MERCHANT_TOTAL_BALANCE']):
            raise ValueError("must be one of enum values ('MERCHANT_DEPOSIT_BALANCE', 'MERCHANT_AVAILABLE_BALANCE', 'MERCHANT_TOTAL_BALANCE')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        if "MerchantResourceInformation".endswith("Response") and "response" in obj and "body" in obj["response"]:
